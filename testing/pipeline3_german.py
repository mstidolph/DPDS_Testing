"""
DPDS Custome Pipeline 3 - German Data
"""
...
def main() -> None:
    ...
    df3['Credit Amount'] = df['Credit Amount'].unique()
    ...
    # Merge with lookup
    df = df.merge(df3, on = ['Credit Amount'])
    ...
    # One hot encoder
    dum = pd.get_dummies(df['Creditability'])
    df = df.join(dum)
    
    # Rename columns
    df = df.rename(columns={"Guarantors": "A", "Telephone": "B"})
    ...
    # Drop feature
    df = df.drop(columns=["Purpose", "Occupation", "Credit Amount"])
