...
class ProvenanceTracker:
    ...
    def _wrapper_track_provenance(self, f, tracker_id: str):
        """
        Wrapper function to track provenance.
        If a method of the TrackedDataframe class is invoked,
        then the wrap function for capturing provenance will be run.
        ...
        def wrap(*args, **kwargs):
            ...
            if callable(f):
                ...
                # If the result is a dataframe then the provenance is captured.
                if isinstance(result, pd.DataFrame) or result is None:

                    # In case of inplace operation.
                    if result is None:
                    ...
                    # Preparing metadata for provenance capture.
    ...
    def __get_prov_space_transformation(self, dataframe_state: DataFrameState,
        ...
        Types of operations captured by this method:
            ...
            - One-Hot Encoding: Multiple binary features added representing categorical values of another feature.
        ...
        function_name8 = "One-Hot Encoding"
        ...
        if len(dropped_cols) == 0 and len(augs_cols) > 0 and len(dropped_rows) == 0 and len(augs_rows) == 0:
            if set(pd.unique(df_output[augs_cols].values.ravel())) == set([0, 1]) and len(augs_cols) > 1:
        ...
        if (len(dropped_cols) > len(augs_cols)) and len(dropped_rows) > len(augs_rows):
        ...
        for index in int_rows:
            ...
            for col_name in augs_cols:
                ...
                # if function_name == function_name8:
                #     col_val = pd.Series({col:df_input[col].unique() for col in df_input})
                    
                #     for col_name2 in col_val.index:
                #         used_entity = dataframe_state.index_col_to_input_entities[(index, col_name2)]
                        
                #         if len(set(augs_cols)) == len(np.unique(col_val[col_name2])):
                            
                #             if set(augs_cols) == set(col_val[col_name2]):
    ...
    def __get_prov_value_change(self, dataframe_state, extra_used_features: set = None) -> None:
        ...
        Ordinal Encoding: the DataFrame column has been encoded.
        ...
        function_name3 = "Ordinal Encoding"
        ...
        dropped_rows = df_input.index.difference(df_output.index)
        drop_index = set()
        for j in range(0, len(dropped_rows)):
            drop_index.add(df_input.index.get_loc(dropped_rows[j]))
        drop_mod = sum(j < df_input.index for j in drop_index)
        drop_mod = np.delete(drop_mod, list(drop_index))
        ...
        for i in df_output.index:
            ...
            for col_name in int_columns:
                ...
                used_entity = dataframe_state.index_col_to_input_entities.get((index_in, col_name), None)
                ...
                if new_value != old_value:
                    ...
                    elif type(old_value) == str and isinstance(new_value, Number):
                        if new_value.is_integer():
                            if old_value.isdigit():
                                if int(old_value) == new_value:
                                    continue
                            else:
                                enc_cols.add(col_name)
                    else:
        ...
        if len(enc_cols) > 0:
            self.logger.info(f' Encoding detect on {enc_cols} columns')
            enc_cols = enc_cols.union(used_features)
            extra_used_entities.extend(used_entities)
            act_id = self.global_state.create_activity(function_name=function_name3, used_features=list(enc_cols),
                                                       description=self.global_state.description,
                                                       code=self.global_state.code,
                                                       code_line=self.global_state.code_line,
                                                       tracker_id=dataframe_state.tracker_id)
        ...
        if len([enc_cols, imp_cols, trans_cols]) == 0:
            self.logger.info(f' Value transformation, Encoding and Imputation not detected')
    ...
    def subscribe(self, df: Union[pd.DataFrame, List[pd.DataFrame]]) -> List[pd.DataFrame]:
        ...
        if isinstance(df, pd.DataFrame):
            # Case where only one dataframe is passed
            ...
            # Case where a list of dataframes is passed
            tracked_dfs = []
            for single_df in df:
                tracker_id = str(uuid.uuid4())
                dataframe_state = DataFrameState(tracker_id=tracker_id)
...
class TrackedDataFrameMeta(type):
    """
    Defines the metaclass for the DataFrameTraked
    ...
    def __new__(cls, name, bases, dct, tracker, tracker_id: str):
        ...
        The wrapper function will take care of tracking the provenance
